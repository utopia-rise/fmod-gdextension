    ...
    static const GDExtensionPropertyInfo* get_property_list_bind(GDExtensionClassInstancePtr p_instance, uint32_t* r_count) {                                    \
        if (p_instance && m_class::_get_get_property_list()) {                                                                                                   \
            if (m_class::_get_get_property_list() != m_inherits::_get_get_property_list()) {                                                                     \
                m_class* cls = reinterpret_cast<m_class*>(p_instance);                                                                                           \
                ERR_FAIL_COND_V_MSG(                                                                                                                             \
                  !cls->plist_owned.is_empty() || cls->plist != nullptr || cls->plist_size != 0,                                                                 \
                  nullptr,                                                                                                                                       \
                  "Internal error, property list was not freed by engine!"                                                                                       \
                );                                                                                                                                               \
                cls->_get_property_list(&cls->plist_owned);                                                                                                      \
                cls->plist = reinterpret_cast<GDExtensionPropertyInfo*>(                                                                                         \
                  memalloc(sizeof(GDExtensionPropertyInfo) * cls->plist_owned.size())                                                                            \
                );                                                                                                                                               \
                cls->plist_size = 0;                                                                                                                             \
                ...
                if (r_count) *r_count = cls->plist_size;                                                                                                         \
                return cls->plist;                                                                                                                               \
            }                                                                                                                                                    \
    ...
    static GDExtensionBool property_get_revert_bind(GDExtensionClassInstancePtr p_instance, GDExtensionConstStringNamePtr p_name, GDExtensionVariantPtr r_ret) { \
        if (p_instance && m_class::_get_property_get_revert()) {                                                                                                 \
            if (m_class::_get_property_get_revert() != m_inherits::_get_property_get_revert()) {                                                                 \
                m_class* cls = reinterpret_cast<m_class*>(p_instance);                                                                                           \
                return cls->_property_get_revert(                                                                                                                \
                  *reinterpret_cast<const ::godot::StringName*>(p_name),                                                                                         \
                  *reinterpret_cast<::godot::Variant*>(r_ret)                                                                                                    \
                );                                                                                                                                               \
            }                                                                                                                                                    \
