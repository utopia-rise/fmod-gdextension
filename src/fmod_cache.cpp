...
void FmodCache::add_plugin(const String& p_plugin_path, uint32_t p_priority) {
    uint32_t handle;
    ...
    plugin_handles[p_plugin_path] = handle;
}

bool FmodCache::has_plugin(const String& p_plugin_path) const {
    return plugin_handles.has(p_plugin_path);
}

void FmodCache::remove_plugin(const String& p_plugin_path) {
    if (!has_plugin(p_plugin_path)) {
        GODOT_LOG_ERROR(vformat("Cannot unload plugin with path %s, not in cache.", p_plugin_path));
    }

    uint32_t handle {plugin_handles[p_plugin_path]};
    ERROR_CHECK(core_system->unloadPlugin(handle));
    plugin_handles.erase(p_plugin_path);
}
