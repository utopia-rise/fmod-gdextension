...
float FmodEvent::get_parameter_by_name(const String& parameterName) const {
    float p = -1;
    ERROR_CHECK(wrapped->getParameterByName(parameterName.utf8().get_data(), &p));
    return p;
}

void FmodEvent::set_parameter_by_name(const String& parameterName, float value) const {
    ERROR_CHECK(wrapped->setParameterByName(parameterName.utf8().get_data(), value));
}
...
float FmodEvent::get_parameter_by_id(const Array& idPair) const {
    ...
    id.data1 = idPair[0];
    id.data2 = idPair[1];
    ERROR_CHECK(wrapped->getParameterByID(id, &value));
    return value;
}
...
void FmodEvent::set_parameter_by_id(const Array& idPair, float value) const {
    ...
    id.data1 = idPair[0];
    id.data2 = idPair[1];
    ERROR_CHECK(wrapped->setParameterByID(id, value));
}

void FmodEvent::release() const {
    wrapped->setUserData(nullptr);
    ERROR_CHECK(wrapped->release());
}

void FmodEvent::start() const {
    ERROR_CHECK(wrapped->start());
}

void FmodEvent::stop(int stopMode) const {
    ERROR_CHECK(wrapped->stop(static_cast<FMOD_STUDIO_STOP_MODE>(stopMode)));
}

void FmodEvent::event_key_off() const {
    ERROR_CHECK(wrapped->keyOff());
}

int FmodEvent::get_playback_state() const {
    int s = -1;
    ERROR_CHECK(wrapped->getPlaybackState((FMOD_STUDIO_PLAYBACK_STATE*) &s));
    return s;
}
...
float FmodEvent::get_pitch() const {
    float pitch = 0.0f;
    ERROR_CHECK(wrapped->getPitch(&pitch));
    return pitch;
}

void FmodEvent::set_pitch(float pitch) const {
    ERROR_CHECK(wrapped->setPitch(pitch));
}
...
int FmodEvent::get_timeline_position() const {
    int tp = 0;
    ERROR_CHECK(wrapped->getTimelinePosition(&tp));
    return tp;
}

void FmodEvent::set_timeline_position(int position) const {
    ERROR_CHECK(wrapped->setTimelinePosition(position));
}

float FmodEvent::get_reverb_level(int index) const {
    float rvl = 0.0f;
    ERROR_CHECK(wrapped->getReverbLevel(index, &rvl));
    return rvl;
}

void FmodEvent::set_reverb_level(int index, float level) const {
    ERROR_CHECK(wrapped->setReverbLevel(index, level));
}

bool FmodEvent::is_virtual() const {
    bool v = false;
    ERROR_CHECK(wrapped->isVirtual(&v));
    return v;
}

void FmodEvent::set_listener_mask(unsigned int mask) const {
    ERROR_CHECK(wrapped->setListenerMask(mask));
}

uint32_t FmodEvent::get_listener_mask() const {
    uint32_t mask = 0;
    ERROR_CHECK(wrapped->getListenerMask(&mask));
    return mask;
}
...
void FmodEvent::set_2d_attributes(Transform2D position) const {
    ...
    ERROR_CHECK(wrapped->set3DAttributes(&attr));
}

Transform2D FmodEvent::get_2d_attributes() const {
    Transform2D _2Dattr;
    FMOD_3D_ATTRIBUTES attr;
    ERROR_CHECK(wrapped->get3DAttributes(&attr));
...
void FmodEvent::set_callback(Callable& callback, int callbackMask) {
    ...
    ERROR_CHECK(wrapped->setCallback(Callbacks::eventCallback, callbackMask));
}
